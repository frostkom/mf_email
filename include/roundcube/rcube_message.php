...
class rcube_message
{
    ...
    public function get_part_url($mime_id, $embed = false)
    {
        if ($this->mime_parts[$mime_id])
            return (isset($this->opt['get_url']) ? $this->opt['get_url'] : "") . '&_part=' . $mime_id . ($embed ? '&_embed=1' : '');
        else
            return false;
    }
    ...
    private function parse_structure($structure, $recursive = false)
    {
        ...
        // message contains (more than one!) alternative parts
            ...
            else if (isset($enriched_part) && $enriched_part !== null) {
        ...
        // message contains multiple parts
            ...
            for ($i=0; $i < count($structure->parts); $i++) {
                ...
                if (isset($mail_part->real_mimetype) && $mail_part->real_mimetype) {
                ...
                if ($primary_type == 'multipart') {
                    ...
                    if (isset($part_orig_mimetype) && $part_orig_mimetype == 'message/rfc822' && !empty($mail_part->filename))
                ...
                // part is a file/attachment
                else if (preg_match('/^(inline|attach)/', $mail_part->disposition) || isset($mail_part->headers['content-id']) && $mail_part->headers['content-id'] || ($mail_part->filename &&  (empty($mail_part->disposition) || preg_match('/^[a-z0-9!#$&.+^_-]+$/i', $mail_part->disposition)))
                ) {
                    ...
                    else if (isset($part_orig_mimetype) && $part_orig_mimetype == 'message/rfc822') {
                        $this->parse_structure($mail_part, true);
            ...
            if ($mimetype == 'multipart/related' && sizeof($this->inline_parts)) {
                ...
                foreach ($this->inline_parts as $inline_object) {
                    ...
                    if (isset($inline_object->content_location) && $inline_object->content_location) {
