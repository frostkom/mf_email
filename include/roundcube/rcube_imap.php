...
class rcube_imap
{
        ...
        // write error log
            ...
            if ($pass && $user) {
                $message = sprintf("Login failed for %s from %s. %s",
                    $user, "", $this->conn->error); //rcmail_remote_ip()
    ...
    private function set_env()
    {
        ...
        if (isset($imap_shared) && $imap_shared !== null) {
    ...
    private function _list_headers($mailbox='', $page=NULL, $sort_field=NULL, $sort_order=NULL, $slice=0)
    {
        ...
        if (!isset($a_msg_headers) || !is_array($a_msg_headers) || empty($a_msg_headers))
            return array();
        ...
        if ($this->sort_order == 'DESC' && (!isset($sorted) || !$sorted))
    ...
    function &_structure_part($part, $count=0, $parent='', $mime_headers=null)
    {
        ...
        */
            ...
            if (isset($mime_part_headers) && $mime_part_headers) {
            ...
            for ($i=0, $count=0; $i<count($part); $i++) {
                if (!is_array($part[$i]))
                    break;
                $tmp_part_id = $struct->mime_id ? $struct->mime_id.'.'.($i+1) : $i+1;
                $struct->parts[] = $this->_structure_part($part[$i], ++$count, $struct->mime_id, isset($mime_part_headers[$tmp_part_id]) ? $mime_part_headers[$tmp_part_id] : "");
            }
        ...
        */
		...
		if((!isset($struct->charset) || empty($struct->charset)) && !empty($mime_headers) && isset($mime_headers->charset) && $mime_headers->charset)
        ...
        if ($struct->ctype_primary == 'message' || (isset($struct->ctype_parameters['name']) && $struct->ctype_parameters['name'] && (!isset($struct->content_id) || !$struct->content_id))) {
    ...
    function &get_message_part($uid, $part=1, $o_part=NULL, $print=NULL, $fp=NULL, $skip_charset_conv=false)
    {
        ...
        if (isset($body) && $body && preg_match('/^(text|message)$/', $o_part->ctype_primary)) {
	...
	//Delete e-mail
	/*function delete_email($messages)
	{
		$mailbox = $this->mailbox;

		$this->conn->delete($mailbox, $messages);
	}*/

	//Read e-mail
	/*function read_email($messages)
	{
		$this->set_flag($messages, 'SEEN');
	}

	//Answered e-mail
	function answered_email($messages)
	{
		$this->set_flag($messages, 'ANSWERED');
	}*/
    ...
    function decode_header($input, $remove_quotes=false)
    {
        ...
        if(isset($str[0]) && $str[0] == '"' && $remove_quotes)
		{
			$str = str_replace('"', '', $str);
		}
