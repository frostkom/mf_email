...
if(!function_exists('is_plugin_active') || function_exists('is_plugin_active') && is_plugin_active("mf_base/index.php"))
{
	...
	if(is_admin())
	{
		...
		add_filter('pre_update_option', array($obj_email, 'pre_update_option'), 10, 3);
		...
		if(wp_doing_ajax())
		{
			add_action('wp_ajax_api_email_smtp_test', array($obj_email, 'api_email_smtp_test'));
		}
	}
	...
	add_filter('get_email_address_from_id', array($obj_email, 'get_email_address_from_id'), 10, 2);
	...
	function activate_email()
	{
		...
		$wpdb->query("CREATE TABLE IF NOT EXISTS ".$wpdb->base_prefix."email (
			...
			emailSmtpVerified TINYINT NOT NULL DEFAULT '0',
		...
		$arr_add_column[$wpdb->base_prefix."email"] = array(
			'emailSmtpVerified' => "ALTER TABLE [table] ADD [column] TINYINT NOT NULL DEFAULT '0' AFTER emailOutgoingType",
		...
		$arr_update_column[$wpdb->base_prefix."email"] = array(
			...
			'emailSmtpVerified' => "ALTER TABLE [table] CHANGE [column] [column] TINYINT NOT NULL DEFAULT '0'", //260325
		);
