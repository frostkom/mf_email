...
if(isset($_POST['btnFolderCreate']) && wp_verify_nonce($_POST['_wpnonce_folder_create'], 'folder_create_'.$intFolderID))
{
	if($intFolderID > 0)
	{
	...
	."<div id='poststuff' class='postbox'>
		...
		<div class='inside'>
			<form".apply_filters('get_form_attr', " action='#'").">";
				...
				$result = $wpdb->get_results("SELECT ".$wpdb->base_prefix."email.emailID, emailName, emailAddress FROM ".$wpdb->base_prefix."email_users RIGHT JOIN ".$wpdb->base_prefix."email USING (emailID) WHERE ".$obj_email->get_permission_where()." AND emailDeleted = '0' ORDER BY emailName ASC, emailAddress ASC");
